...
export class CardViewComponent implements OnInit {
  ...
  constructor(private shareService: ShareService, private dbService: dbService, private route: ActivatedRoute, private router: Router, private searchService: SearchService, private firebaseAuth: AngularFireAuth, private notifierService: NotifierService) {
  ...
  addToSavedCard(cardName: string, cardAvatar: string, cardId: string, cardType: string) {

    let savedCardDetails = { name: cardName, avatar: cardAvatar, id: cardId, type: cardType }
    console.table(savedCardDetails);
    this.stateLoading = true;
    // console.log(savedCardDetails);
    this.dbService.addToSavedCards(this.user.uid, savedCardDetails, cardId).then(res => {
  ...
  likeThisCard(cardName: string, cardAvatar: string, cardId: string, cardType: string) {

    this.showLike = !this.showLike
    let savedCardDetails = { name: cardName, avatar: cardAvatar, id: cardId, type: cardType }
    //console.table(savedCardDetails);

    this.dbService.likeCard(this.user.uid, savedCardDetails, cardId).then(res => {
      this.dbService.updateLike(this.user.uid, cardId).then(res => {
        this.dbService.updateLike(this.user.uid, cardId).then(res => {
          this.fetchCardDetails(this.cardId);
        }).catch(err => {
          this.showLike = this.showLike;
          this.notifier.notify('error', 'unable to like the card, please try again');
          console.log(err)
        })
      })
    })
  ...
  unLikeThisCard(cardId: string) {
    ...
    this.showLike = !this.showLike;
    this.dbService.unlikeCard(this.user.uid, cardId).then(res => {
  ...
  fetchCardDetails(cardId: string) {
    this.dbService.getCardData(cardId).subscribe(res => {
      ...
      if (this.cardData) {
        ...
        this.remaingLinksCount = Object.keys(this.cardData.socialLinks).length - 5;
        this.stateLoading = false;
        ...
        this.shareService.setTwitter(window.location.href,
          `Hey I am ${this.cardData.userName}, checkout my internet card here`,
          "Internet card lets you to mange and share all the business and personal social media links at one place.")
      }
      else {
  ...
  goToCardEdit() {
    this.router.navigateByUrl(`/card/edit/${this.cardId}?cardType=${this.cardData.cardType}`)
  }
  ...
  copyToClipboard() {
    ...
    this.notifier.notify('success', 'Card url is copied to clipboard successfully');
  ...
  ngOnInit(): void {
    ...
    this.searchService.getData().subscribe(data => {
      ...
      if (data) {
        ...
        this.stateLoading = true;
        this.dbService.searchCards(this.myData.toLowerCase()).subscribe(res => {
