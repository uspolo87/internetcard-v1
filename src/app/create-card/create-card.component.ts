...
export class CreateCardComponent implements OnInit {
  ...
  cardBg: string = "false";
  cardBgColor: any;
  card: any = { userProfession: "student", socialLinks: {}, userGender: "gender", userCountry: "India" };
  userName: string = "";
  cardType: any;
  skillSearch: any;
  skillsArray: any = [];
  ...
  constructor(private route: ActivatedRoute, private router: Router, private dbService: dbService, private firebaseAuth: AngularFireAuth, private authService: AuthService, private notifierService: NotifierService, private storage: AngularFireStorage) {
  ...
  imageCropped(event: ImageCroppedEvent) {
    ...
    this.businessProfileImage = new File([blob], `businessProfile-${this.user.uid}`);
  }
  ...
  uploadCv() {
    ...
    this.storage.upload(`resume/${this.user.uid}`, this.filePath).then(res => {
  ...
  uploadBusinessProfilePicture() {
    this.stateLoading = true;
    this.storage.upload(`businessLogo/${this.user.uid}`, this.businessProfileImage).then(res => {
      res.ref.getDownloadURL().then(url => {
  ...
  updateUserBasicInfo() {
    ...
    if (this.cardType === 'business') {
      ...
      userBasicDetails.search = this.card.userName.toLowerCase().split(" ") || "";
    }
  ...
  async createCard() {
    ...
    this.dbService.createCard(this.card, this.user.uid).then(res => {
  ...
  showStep(step: number) {
    if (this.cardType === "personal") {
      ...
      if (step === 2) {
        ...
        if (!this.card.userName || !this.card.userContact || !this.card.gmail || !this.card.userCountry || !this.card.userGender || !this.card.userDob) {
      ...
      if (step === 4) {
        ...
        if (this.card.userProfession === "student") {
          if (!this.card.userBio || !this.card.userInstitute || this.skillsArray.length === 0) {
            this.notifier.notify('error', 'Please fill all the details');
            return;
          }
        }
        else if
          (!this.card.userBio || !this.card.userCompany || this.skillsArray.length === 0) {
          this.notifier.notify('error', 'Please fill all the details');
          return;
        }
      ...
      if (step === 2) {
        if (!this.card.businessName || !this.card.businessContact || !this.card.businessEmail || !this.card.businessCountry || !this.card.businessCity) {
          this.notifier.notify('error', 'Please fill all the details');
          return;
        }
      }
