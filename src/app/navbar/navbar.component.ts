...
export class NavbarComponent implements OnInit, OnDestroy {
  ...
  constructor(private router: Router, private searchService: SearchService, private route: Router, private ref: ChangeDetectorRef, public authService: AuthService, private dbService: dbService, private firebaseAuth: AngularFireAuth) {
    firebaseAuth.authState.subscribe(user => {
      ...
      if (user) {
        ...
        if(window.location.href.includes(user.uid) && window.location.href.includes("cardView") ){
    ...
    this.router.events.subscribe(
      ...
      (event) => {
        ...
        if (event instanceof NavigationStart) {
          ...
          if (event.url === "/login" || event.url === "/terms" || event.url === "/privacy") {
            this.hideNavbarFunc();
          }
          else {
            this.showNavbarFunc();
          ...
          if( window.location.href.includes(userId) && event.url.includes("cardView")){
            this.hideCardViewBtn = true;
          }
          else{
