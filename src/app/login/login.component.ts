...
export class LoginComponent implements OnInit {
  ...
  loginWithCredentials(userCredentials: any) {
    ...
    this.authService
      .login(
        userCredentials.value.userName,
        userCredentials.value.userPasssword
      )
      .then((res) => {
        this.user = res.user;
  ...
  getUserInfo(user: any) {
    this.dbService.getUserInfo(user.uid).subscribe((res) => {
      ...
      if (this.cardCreated) {
        ...
        this.dbService
          .saveUserInfo(this.user.uid, userDetailsObj)
          .then((res) => {
  ...
  signUpWithCredentails(signUpFormData: any) {
    ...
    if (
      signUpFormData.value.userSignupName === '' ||
      signUpFormData.value.userSignupPassword === '' ||
      signUpFormData.value.userSignupConfirmPassword === ''
    ) {
    ...
    if (!re.test(signUpFormData.value.userSignupPassword)) {
      this.notifier.notify(
        'error',
        'password should be min 8 letters, with at least a symbol, upper and lower case letters and a number'
      );
      return;
    }
    if (
  ...
  sendPasswordResetLink(formData: any) {
    this.authService
      .sendPasswordResetLink(formData.value.userPasswordResetemail)
      .then(
        (res) => {
